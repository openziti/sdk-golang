...
func (self *newConnHandler) dialFailed(err error) {
	...
	if err := reply.WithTimeout(5 * time.Second).SendAndWaitForWire(self.ctrlSender); err != nil {
...
func (self *newConnHandler) dialSucceeded() (error, bool) {
	...
	} else if err := reply.WithTimeout(time.Second * 5).SendAndWaitForWire(self.ctrlSender); err != nil {
		logger.WithError(err).Error("failed to send reply to dial request")
		return err, false
	}
