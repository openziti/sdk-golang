...
// newTestPath builds a RouterChannelPath over a counting sender. It gets a real
// conn mux so the path's Close can deregister, as it does in production.
...
// newTestAdapter builds an adapter with an xgress attached, so the path-presence
// notifications AddPath and RemovePath emit have somewhere to land. The xgress is
// deliberately not started: these tests exercise adapter bookkeeping, not the
// send-buffer run loop.
func newTestAdapter(initial Path) *MultiPathAdapter {
	adapter := NewMultiPathAdapter("c1", newTestEnv(), SinglePathSelector{}, initial)
	xg := xgress.NewXgress("c1", "ctrl", "addr", nil, xgress.Initiator, xgress.DefaultOptions(), nil)
