...
// AddPath attaches a path to this adapter. The caller is responsible for
// having registered the path's receive sink first, so inbound traffic on the
// new path has somewhere to land before outbound dispatch can select it.
//
...
func (self *MultiPathAdapter) AddPath(path Path) {
	self.lock.Lock()
...
func (self *MultiPathAdapter) HandleXgressClose(x *xgress.Xgress) {
	...
	// can't arm a stray hold, and cancel any hold already in progress
	self.lock.Lock()
	self.recoverable = false
	self.cancelHoldLocked()
	self.lock.Unlock()
