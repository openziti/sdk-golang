...
func (conn *edgeConnXgress) setupXgressFlowControl(msg *channel.Message, originator xgress.Originator,
	...
	// NB: the xgress is intentionally NOT started here. The caller must register
	// this conn as a mux sink (mux.Add) and THEN call start(). Starting the
	// xgress sends CircuitStart, which releases terminator-side data; if the
	// sink weren't registered first, an inbound payload/ack/control could arrive
	// with no sink and be dropped. See connect-v2 review C1.
	return nil
}

// start launches the xgress. It must be called only after the conn has been
// registered as a mux sink, so inbound messages have somewhere to dispatch the
// moment the xgress goes live (and, on the initiator, sends CircuitStart).
