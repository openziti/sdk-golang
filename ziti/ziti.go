...
type Context interface {
	...
	GetServiceTerminators(serviceName string) ([]*edge.Terminator, error)
...
func (context *contextImpl) GetServiceTerminators(serviceName string) ([]*edge.Terminator, error) {
	service, found := context.GetService(serviceName)
	if !found {
		return nil, errors.Errorf("did not find service named %v", serviceName)
	}
	return context.ctrlClt.GetServiceTerminators(service)
}
